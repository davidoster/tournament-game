...
namespace TrackerUI
{
    public partial class CreatePrizeForm : Form
    {
        ...
        private bool ValidateForm()
        {
            ...
            double prizePercetage = 0;

            bool prizeAmountValid = decimal.TryParse(prizeAmountValue.Text, out prizeAmount);
            bool prizePercetageValid = double.TryParse(prizePercentageValue.Text, out prizePercetage);

            if (prizeAmountValid == false || prizePercetageValid == false)
            {
                output = false;
            }

            if (prizeAmount <= 0 && prizePercetage <= 0)
            {
                output = false;
            }

            if (prizePercetage < 0 || prizePercetage > 100)
            {
                output = false;
            }
